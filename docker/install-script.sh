...
if ! [ -f "${HOME}/SGC_Stage1" ]; then
    touch "${HOME}/SGC_Stage1"
fi
...
/usr/local/games/steamcmd/steamcmd.sh +set_steam_guard_code ${STEAM_GUARD_CODE} +login ${STEAM_USER} ${STEAM_PASS} +log_files_always_flush 1 +@ShutdownOnFailedCommand 1 +quit 2>&1 | tee ${HOME}/SGC_Stage1
LastExitState=$?
...
if [ $LastExitState -eq 5 ] && [ "$LastExitMessage" == "Invalid Login Auth Code" ]; then
    ...
    /usr/local/games/steamcmd/steamcmd.sh +login ${STEAM_USER} ${STEAM_PASS} +log_files_always_flush 1 +@ShutdownOnFailedCommand 1 +quit 2>&1 >${HOME}/SGC_Stage2 &
